...
本文记录了把 QuillNote（一款基于 Tauri v2 的桌面 Markdown 编辑器）发布到微软商店（Microsoft Store）的完整过程，包含 MSIX 打包、Partner Center 配置、GitHub Actions 自动发布的全流程，以及途中踩过的所有坑。
...
QuillNote 的默认分发方式是 NSIS（`.exe`）+ MSI（`.msi`）+ macOS/Linux 包，通过 GitHub Releases 发布。但微软商店**只接受 MSIX 格式**，且 Tauri CLI 2.11.2 在 Windows 平台的 `--bundles` 不支持 `msix` 目标（只有 `msi`/`nsis`）。
...
输入产品名「**QuillNote**」并保留（名字保留后 3 个月内必须发布，否则失效）。
...
| 字段 | 示例值 | 用途 |
| --- | --- | --- |
| Package/Identity/Name | `<你的Name>`（形如 `1234567890.QuillNote`） | MSIX 清单的 `Identity Name` |
...
2. **读取 release exe**：`src-tauri/target/release/quillnote.exe`
3. **暂存目录**：`src-tauri/target/msix-staging/`，把 exe + 图标 + 生成的 manifest 放进去
4. **MakeAppx pack**：打包成 `src-tauri/target/msix/QuillNote_<version>_x64.msix`
...
> [!WARNING].env 里的值不要加引号。QuillNote 的 loadEnv 解析器只按第一个 = 分割并 trim 两边，不会剥引号。如果写成 MSSTORE_PACKAGE_IDENTITY_NAME="你的Name"，引号会被当作值的一部分传给 MakeAppx，导致 Partner Center 校验 Identity Name 时报错。
> 这是本文踩过的第二个坑——无效的软件包标识名称: QuillNote (应为: 你的Name)，实际上是因为之前 .env 里写成了 = " 你的Name"（带前导空格和引号）。
...
✓ MSIX 已生成: D:\code\QuillNote\src-tauri\target\msix\QuillNote_0.1.4.0_x64.msix
  大小: 6.09 MB
```
...
进入 QuillNote 应用页面 → **Start a submission**（开始新提交）。需要填的几个块：
...
在 **Packages** 块，把本地生成的 `QuillNote_0.1.4.0_x64.msix` 拖进去。
...
```
无效的软件包标识名称: QuillNote (应为: <你的Name>)
无效的软件包发布者名称: CN=QuillNote (应为: <你的Publisher>)
```
...
> QuillNote 是一款桌面 Markdown 编辑器，采用 Tauri v2 框架构建，核心工作模式为「本地优先」——所有笔记、文件、配置都保存在用户自己的设备上，不经过任何云端服务器。
>
...
微软商店**强制要求**所有应用提供隐私策略 URL。我在文档站专门加了一个页面：[隐私策略 · QuillNote](https://Rankine-L.github.io/QuillNote/01-%E5%BC%80%E5%A7%8B%E4%BD%BF%E7%94%A8/%E9%9A%90%E7%A7%81%E7%AD%96%E7%95%A5)
...
| Name | Value |
| --- | --- |
| `MSSTORE_PACKAGE_IDENTITY_NAME` | `<你的Name>`（如 `1234567890.QuillNote`） |
...
QuillNote 的 `loadEnv` 解析器不剥引号，导致实际传给 MakeAppx 的 Name 是带引号带空格的字符串，触发 Partner Center 校验失败。
...
```
应用清单中的 PublisherDisplayName 元素是 QuillNote，该值与发布者显示名称: <你的发布者显示名> 不匹配
```

**根因**：Partner Center 注册时填的发布者显示名（个人账号默认用真实姓名），但 manifest 模板里硬编码了「QuillNote」。
...
- \[\[08-高级功能/01-发布网站\]\] —— QuillNote 内置的静态网站发布功能
- \[\[08-高级功能/04-自动更新配置\]\] —— GitHub Releases 自动更新签名配置
- \[\[01-开始使用/02-关于\]\] —— QuillNote 版本信息与技术栈
