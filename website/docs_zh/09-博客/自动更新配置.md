...
QuillNote 支持自动更新功能，用户启动应用时会自动检查并安装最新版本。本文档介绍如何配置自动更新所需的签名密钥。
...
生成两个文件：
- `~/.tauri/quillnote.key` — 私钥（保密）
- `~/.tauri/quillnote.key.pub` — 公钥（公开）

## 配置 GitHub Secrets

### 步骤

1. 打开仓库 `Rankine-L/QuillNote`
...
| Secret 名称 | 值 | 说明 |
|------------|-----|------|
| `TAURI_SIGNING_PRIVATE_KEY` | 私钥文件内容 | `cat ~/.tauri/quillnote.key` 的输出 |
...
| 文件 | 说明 |
|------|------|
| `QuillNote_x.x.x_x64-setup.exe` | Windows 安装包 |
| `QuillNote_x.x.x_x64-setup.exe.sig` | Windows 签名文件 |
| `QuillNote_aarch64.app.tar.gz` | macOS ARM 安装包 |
| `QuillNote_aarch64.app.tar.gz.sig` | macOS ARM 签名文件 |
| `QuillNote_amd64.AppImage` | Linux 安装包 |
| `QuillNote_amd64.AppImage.sig` | Linux 签名文件 |
